...
ifneq ($(findstring $(REPOSITORY_MAKE_DOLLAR)$(REPOSITORY_MAKE_OPEN_BRACE),$(value NODE)),)
$(error NODE must be a literal executable path, not Make syntax)
endif
...
ifneq ($(findstring $(REPOSITORY_MAKE_DOLLAR)$(REPOSITORY_MAKE_OPEN_BRACE),$(value NPM)),)
$(error NPM must be a literal executable path, not Make syntax)
endif
...
override REPOSITORY_NODE_LITERAL := $(call REPOSITORY_SHELL_LITERAL,$(NODE))
override REPOSITORY_NPM_LITERAL := $(call REPOSITORY_SHELL_LITERAL,$(NPM))
...
lint::
	'$(REPOSITORY_NODE_LITERAL)' '$(REPOSITORY_ROOT_LITERAL)/scripts/check_wedding_contracts.js'

test::
	'$(REPOSITORY_NODE_LITERAL)' '$(REPOSITORY_ROOT_LITERAL)/scripts/check_wedding_contracts.js'
	@if [ -d '$(REPOSITORY_ROOT_LITERAL)/app/node_modules' ]; then \
		'$(REPOSITORY_NPM_LITERAL)' --prefix '$(REPOSITORY_ROOT_LITERAL)/app' test; \
	else \
		/usr/bin/printf '%s\n' 'Skipping npm test: app/node_modules is not installed.'; \
	fi

build:: lint
	@if [ -d '$(REPOSITORY_ROOT_LITERAL)/app/node_modules' ]; then \
		'$(REPOSITORY_NPM_LITERAL)' --prefix '$(REPOSITORY_ROOT_LITERAL)/app' run build; \
	else \
		/usr/bin/printf '%s\n' 'Skipping CSS build: app/node_modules is not installed.'; \
	fi
