...
const expectedExternalResources = new Map([
  ...
  ['https://cdnjs.cloudflare.com/ajax/libs/less.js/2.7.1/less.min.js', 'sha384-CxtfC0hc2gDgMW/e3Sdq+UI3CxRv3A/DwBYrQ2c/mmbY3GASa6G5A/AxIt+mFF77'],
...
assert(workflowSource.includes('workflow_dispatch:'), 'CI must support manual dispatch');
assert(workflowSource.includes('concurrency:'), 'CI must cancel superseded runs');
assert(workflowSource.includes('cancel-in-progress: true'), 'CI must enable concurrency cancellation');
